...
public class GameUIController : MonoBehaviour, IGameUIController
{
    ...
    private void SetupButtons()
    {
        ...
        {
            _newGameButton.onClick.AddListener(() => _signalBus.Fire<StartGameSignal>());
        }
    }
    ...
    public void ShowRoundResult(GameRoundResult result)
    {
        if (_roundResultText == null) return;

        string resultText = result.Result switch
        {
            GameResult.PlayerWins => $"You win this round! (+{result.CardsWon} cards)",
            GameResult.OpponentWins => $"Opponent wins this round! (-{result.CardsWon} cards)",
    ...
    public void ShowGameResult(GameEndResult result)
    {
        ...
        {
            string winnerText = result.PlayerWon ? "YOU WIN!" : "OPPONENT WINS!";
            _gameEndText.text = $"{winnerText}\nRounds played: {result.TotalRounds}";
        }
        
        Debug.Log($"GameUI: Game ended - {result.WinnerName} won after {result.TotalRounds} rounds");
    }
